<?php
// ----------------------------------------------------------------------------
// Zoom Search Engine 8.0 (13/Aug/2021)
// PHP search script
// A fast custom website search engine using pre-indexed data files.
// Copyright (C) PassMark Software 2000 - 2020
//
// This script is designed for PHP 5.4+ only.
//
...
// details: https://www.zoomsearchengine.com/zoom/usersguide.html
// 
// IF YOU NEED TO ADD PHP TO THE SEARCH PAGE, see this FAQ:
// https://www.zoomsearchengine.com/zoom/support/faq_ssi.html
//
// help@passmark.com
...
function ShowTemplate()
{
	...
	// for a solution: https://www.zoomsearchengine.com/zoom/support/faq_ssi.html
...
function RecLinkWordMatch($rec_word, $rec_idx)
{
	...
	global $SearchWords, $UseWildCards, $SearchAsSubstring, $ToLowerSearchWords, $RegExpSearchWords;
	...
	if ($FormFormat == 2)
	{
		...
		if ($DateRangeSearch == 1)
		{
			...
			if ($DateRangeFormat == 1)
				...
				$dateformatStr = "dmy";
			$OutputBuffers[$OUTPUT_FORM_DATERANGE] .= "<label for=\"zoom_datefrom\">". $STR_FORM_DATE_FROM . " </label><input id=\"zoom_datefrom\" name=\"zoom_datefrom\" value=\"".htmlspecialchars($date_from, ENT_COMPAT, $Charset)."\"><input type=\"button\" value=\"".$STR_FORM_DATE_BUTTON."\" onclick=\"displayDatePicker('zoom_datefrom', false, '".$dateformatStr."');\" />";
			$OutputBuffers[$OUTPUT_FORM_DATERANGE] .= "<label for=\"zoom_dateto\">".$STR_FORM_DATE_TO . " </label><input id=\"zoom_dateto\" name=\"zoom_dateto\" value=\"".htmlspecialchars($date_to, ENT_COMPAT, $Charset)."\"><input type=\"button\" value=\"".$STR_FORM_DATE_BUTTON."\" onclick=\"displayDatePicker('zoom_dateto', false, '".$dateformatStr."');\" />";
...
if (empty($query) && $IsZoomAC == 0)
{
	...
	if ($NoSearch)
	{
		...
		if ($ZoomInfo == 1)
			$OutputBuffers[$OUTPUT_PAGENUMBERS] .= "<p style=\"text-align: center;\"><small>" . $STR_POWEREDBY . " <a href=\"https://www.zoomsearchengine.com/zoom/\" target=\"_blank\"><b>Zoom Search Engine</b></a></small></p>";
...
$UseBroadNumericMatching = 0;
if (isset($BroadNumericValidChars) && strlen($BroadNumericValidChars) > 0)
{
	if (isset($BroadNumericStripChars) && strlen($BroadNumericStripChars) > 0)
	{
		$BroadNumericStripCharsArray = str_split($BroadNumericStripChars);
		if (count($BroadNumericStripCharsArray) > 0)
			$UseBroadNumericMatching = 1;
	}	
}
...
$query = str_replace(array("\xe2\x80\x9c", "\xe2\x80\x9d", "\xa1\xa7", "\xa1\xa8"), "\"", $query);	// smart double quotes
$query = str_replace(array("\xe2\x80\x98", "\xe2\x80\x99", "\xa1\xa5", "\xa1\xa6"), "'", $query);	// smart single quote
...
for ($sw = 0; $sw < $NumSearchWords; $sw++)
{
	...
	if ($UseBroadNumericMatching == 1)
	{		
		if (strspn($SearchWords[$sw], $BroadNumericValidChars) == $swlen)
		{
			$SearchWords[$sw] = str_replace($BroadNumericStripCharsArray, "", $SearchWords[$sw]);
		}
	}
...
{	
	...
	if ($UseMetaFields == 1 && $DisplayMetaFields == 1)
	{
		for ($fieldnum = 0; $fieldnum < $NumMetaFields; $fieldnum++)
		{
			...
			if ($metafields[$fieldnum][$METAFIELD_TYPE] == $METAFIELD_TYPE_MULTI)
			{
				if (isset($metavalues[$ipage][$fieldnum][0]) && $metavalues[$ipage][$fieldnum][0] > 0)
				{
...
// PHP4 support and additional features by PassMark, copyright 2009.
