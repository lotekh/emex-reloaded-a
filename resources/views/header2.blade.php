        ...
        <ul class="navigation_ul row align-center gap-md font-500">
            ...
            <li class="dropdown products-dropdown">
                <span class="dropdown-toggle menuitm" data-toggle="dropdown">
                    Aplicare
                    <amp-img src="{{ asset('resources/new_design/icons/expand_more.svg') }}" height="24" width="24" alt="See more" title="See more"></amp-img>
                </span>
                <div class="dropdown-menu">
                ...
                <div class="dropdown-menu">
                    <ul class="category-wrapper">
                        ...
                        @foreach ($categories as $ind => $category)
                            @php
                            $category_product = \App\Models\CategoryProduct::where('category_id', $category->id)->first();
                            $slug = \App\Models\Slug::where('entity_id', $category_product->product_id)->where('type', 'consum')->first();
                            @endphp
                            @if ($category_product && $slug)
                                <li class="{{ $ind % 2 > 0 ? 'blue-item' : '' }}">
