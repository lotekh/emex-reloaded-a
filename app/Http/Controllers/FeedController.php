    ...
    public function compari()
    {
        ...
        foreach ($products as $product) {
            ...
            if ($db_product) {
                ...
                $contents .= $product['compari_category'] . ', Romtehnochim SRL,' . $product['title'] . ',' . $description . ', ' . $product['price'] . ', ' . ('https://emex.ro/storage/' . $db_product->largeImage->path) . ', ' . ('https://emex.ro/' . $product['link'])  . ', 3 zile, 50,' . $product['id'] . ', ' . $product['gtin'] . ", \r\n";
                $count++;
            }
        }
